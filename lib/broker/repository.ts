...
export class RpcRepository {
  ...
  async listenQ(
    queue: string,
    callback: (replyQueue: string, msg: ConsumeMessage | null) => Promise<void>
  ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        channel.cancel(consumerTag).catch((e) => {
          logger.error(("0" + e) as string, "RPC Repository", true);
        });
      ...
      const onReply = (replyMsg: ConsumeMessage | null) => {
        if (replyMsg?.properties.correlationId === correlationId) {
          ...
          try {
            ...
            logger.error(("2" + e) as string, "RPC Repository", true);
            reject(e);
          }
        }
      };
